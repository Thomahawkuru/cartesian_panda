...
#include <cartesian_panda/cartesian_impedance_controller.h>
...
#include <cartesian_panda/pseudo_inversion.h>
...
bool CartesianImpedanceController::init(hardware_interface::RobotHW* robot_hw,
  ...
  dynamic_server_compliance_param_ = std::make_unique<
      dynamic_reconfigure::Server<cartesian_panda::compliance_paramConfig>>(
...
void CartesianImpedanceController::update(const ros::Time& /*time*/,
  ...
  //cap of maximum error
  double max_lin_error = 0.03; //ERF: 0.025
  double max_ang_error = 2; 

  for (int i = 0; i < 3; i++)
  {
    error[i] = std::max(std::min(error[i],max_lin_error),-max_lin_error);
  } 

  for (int i = 4; i < 7; i++)
  {
    error[i] = std::max(std::min(error[i],max_ang_error),-max_ang_error);
  } 
...
void CartesianImpedanceController::complianceParamCallback(
    cartesian_panda::compliance_paramConfig& config,
    uint32_t /*level*/) {
...
PLUGINLIB_EXPORT_CLASS(cartesian_panda::CartesianImpedanceController,
